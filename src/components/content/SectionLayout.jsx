...
import { getAdjacentSections } from '../../subjects';
...
function SectionLayout({
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {/* Main content */}
      <main className="prose-cpp max-w-none">
        {children}
      </main>
      ...
      <footer className="mt-12 space-y-6 border-t border-gray-200 pt-8 dark:border-gray-700">
        ...
        <nav className="flex items-stretch justify-between gap-4">
          {prev ? (
            <button
              onClick={() => navigate(buildSectionPath(prev))}
              className="group flex max-w-xs flex-1 items-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3 text-left shadow-sm transition-all hover:border-blue-300 hover:shadow-md dark:border-gray-700 dark:bg-gray-800/30 dark:hover:border-blue-600"
            >
          ...
          {next ? (
            <button
              onClick={() => navigate(buildSectionPath(next))}
              className="group flex max-w-xs flex-1 items-center justify-end gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3 text-right shadow-sm transition-all hover:border-blue-300 hover:shadow-md dark:border-gray-700 dark:bg-gray-800/30 dark:hover:border-blue-600"
            >
              <div className="min-w-0">
                <p className="text-xs text-gray-400 dark:text-gray-500">Next</p>
                <p className="truncate text-sm font-medium text-gray-700 dark:text-gray-300">
                  {next.title || next.sectionId}
                </p>
              </div>
              <svg className="h-5 w-5 shrink-0 text-gray-400 transition-transform group-hover:translate-x-1 dark:text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
